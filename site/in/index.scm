...
(h4 "Features")
(p "The architecture indipendent features (not updated) are:"
   (ul
    (li "Strict architecture-independant layer that allows clean porting to other architectures")
    (li "Integrated debugger support")
    (li "Symbolic backtraces (via a BFD-like approach)")
...
(br)
(p "The Intel IA-32 (x86) supports the following features/hardware (not updated):")
(ul
 (li "IDT / GDT")
 (li "PIT (i8253)")
 (li "BIOS")
...
(p "If you think you have found a bug in Hybrid then please send as complete "
   "a report as possible to <hybrid AT nongnu.org>. An easy way to collect all "
